...
long.dat <- read_csv("Data/All Point measurements.csv", skip = 4,                         # read in the EM csv
...
#### MaxN ####
maxn.dat <- read_csv("Data/All Point measurements.csv", skip = 4,      # read in the EM csv
                     col_types = cols(`Camera no.` = col_character())) %>%                # this column has letters and numbers, this part of the code makes sure it's a character, not an integer.
  dplyr::select(1:3, 10:11, 14:18, 23:24, 26:28) %>%                                      # selects all the columns that aren't empty or useful
  unite(spp, Genus, Species, sep = ".") %>%                                               # creates a new column that combine the Genus and Species columns
  add_column(count = 1) %>%                                                               # adds a column of all 1's, this is how we get MaxN per frame
...
write.csv(maxn.dat, "Data/proj-fish-maxn-20260527.csv", row.names = FALSE)                 # saves the data frame as a .csv file, add project and date to filename
...
obs.data <- read_csv("Data/All Point measurements.csv", skip = 4,
...
write.csv(obs.data, "Data/proj-fish-obs-20260527.csv", row.names = FALSE)
...
behav.long <- read_csv("Data/All Point measurements.csv", skip = 4,
                 col_types = cols(`Camera no.` = col_character())) %>%
...
write.csv(behav.long, "Data/behaviour-20260527.csv", row.names = F)

## Feeding, this creates a data frame similar to the MaxN and Obs frame but for observed bites.
feed.dat <- read_csv("Data/All Point measurements.csv", skip = 4,                         # read in the EM csv
                     col_types = cols(`Camera no.` = col_character())) %>%                # this column has letters and numbers, this part of the code makes sure it's a character, not an integer.
  dplyr::select(1:3, 10:11, 14:18, 23:24, 26:28) %>%                                      # selects all the columns that aren't empty or useful
  unite(spp, Genus, Species, sep = ".") %>%
...
write.csv(feed.dat, "Data/bites-20260527.csv", row.names = F)

### Feeding - long data frame.
feed.long <- read_csv("Data/All Point measurements.csv", skip = 4,                         # read in the EM csv
                     col_types = cols(`Camera no.` = col_character())) %>%                # this column has letters and numbers, this part of the code makes sure it's a character, not an integer.
  dplyr::select(1:3, 10:11, 14:18, 23:24, 26:28) %>%                                      # selects all the columns that aren't empty or useful
  unite(spp, Genus, Species, sep = ".") %>%
  filter(Activity == "Feeding") %>%                                                       # selects on the Feeding data points
...
write.csv(feed.long, "Data/bites-long-20260527.csv", row.names = F)
